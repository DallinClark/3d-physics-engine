...
void Setup::makeAngryBirds(std::shared_ptr<World> engine, Texture& groundTexture, Texture& boxTexture, Texture& sphereTexture, std::shared_ptr<Camera> camera) {
    ...
    success = RigidBody::CreateTetrahedronBody(5.0f,3.0f,3.0f, glm::vec3(-5.9f, 7.5f,2.8f), 1.0f, false, 0.5f, body, errorMessage, engine->getTetrahedronMesh(), boxTexture);
    if (!success) {
        std::cerr << "Failed to create RigidBody: " << errorMessage << std::endl;
    }
    ...
    success = RigidBody::CreateTetrahedronBody(5.0f,1.0f,1.0f, glm::vec3(-5.0f, 30.5f,3.0f), 1.0f, false, 0.5f, body, errorMessage, engine->getTetrahedronMesh(), boxTexture);
    if (!success) {
        std::cerr << "Failed to create RigidBody: " << errorMessage << std::endl;
    }
    body->rotate(45, glm::vec3(0.3f, 0.4f, 0.0f));
